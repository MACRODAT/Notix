...
class block
{
    ...
    addTransaction(transaction : transaction) : boolean // inserted ?
    {
        ...
        console.log(transaction.isTransactionValid);
...
class blockchain{
    ...
    computeLatestHash()
    {
        ...
        this.latestHash = this.blocks[this.blocks.length - 1 ].computeHash(null);
    }
    ...
    addTransaction(transaction_ : transaction) : boolean
    {
        ...
        if (this.invalidatedBlocks !== null)
        {
            ...
            this.blocks[this.blocks.length - 1].addTransaction(transaction_);
            return true;
        }
    }
